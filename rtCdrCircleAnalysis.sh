...
# Description    :- CDR-CIRCLE-ANALYSIS                                       #
# Owner          :- Divyani Mittal                                            #
# Last Updated   :- 29 June 2017                                              #
###############################################################################
# Script  Inputs:
#
#   1. GUI-ARGUMENT OPTION       : 1 or 2
#   --- SELECTED GUI REDIRECTED TO GUI ----
#   ----  SELECTED ARGUMENT ENTER BELOW PARAMETERS ---
#   2. RELATIVE-ABSOLUTE OPTION  : 1 or 2
#   --- SELECTED RELATIVE  ----
#   3. MINUTES                   : 30
#   4. PHONE NUMBER              : 99XXXXXXXXXX
#   --- SELECTED ABSOLUTE ----
#   3. START TIME                : YYYYMMDDHHMMSSMMM
#   4. END TIME                  : YYYYMMDDHHMMSSMMM
#   5. PHONE NUMBER              : 99XXXXXXXXXX
...
RT_CLUSTER_INVOKE_SCRIPT=$RT_SCRIPT_PATH/circle_ip_details.txt


##############
#Options Ist Parameter 1 : Relative Total 2 Parameters Relative time in minutes  pno
#Options 2nd Parameter 2 : Absolute Enter 3 Parameters start end pno
####################
...
#END_DURATION=$2
#PHONE_NUMBER=$3
#ACTIVE_TRIGGER_CONTROLLER_IP=$4
SELF_IP_ADDRESS=` hostname -I | awk '{print $1}'`
ACTIVE_TRIGGER_CONTROLLER_IP=$SELF_IP_ADDRESS


#Error Code
export  PROCESS_ALREADY_RUNNING=0
export HELP_EXIT=1
export END_TIME_NOT_GIVEN=2
export PNO_NOT_GIVEN=3
export START_TIME_NOT_GIVEN=4
export INVALID_ARGUMENTS=5
export RELATIVE_MINUTES_NOT_GIVEN=6
export NO_CSV_FILTERED_OUT=7
export MIN_EXCEED_LIMIT=8
export INVALID_HELP_OPTION_CHOSEN=9
export GUI_MINUTES_NOT_GIVEN=10
export GUI_PHONE_NUMBER_NOT_GIVEN=11
export GUI_START_TIME_NOT_GIVEN=12
export GUI_END_TIME_NOT_GIVEN=13
export INVALID_MINUTES=14
export INVALID_START_TIME=15
export INVALID_END_TIME=16
export INVALID_PHONE_NUMBER=17
export GUI_ARGUMENT_OPTION_NOT_GIVEN=18
export INVALID_GUI_ARG_OPTION_SELECTED=19

#cluster_1_active_controller_ip=10.64.220.2
#cluster_2_active_controller_ip=10.64.220.12
#cluster_3_active_controller_ip=10.64.220.2
#4th Cluster is the trigger Cluster Itself

#PATH Variables :

PATH_FINAL_COLLECTOR=$PATH_OUTPUT_CDR/number_anasysis/circlewise_report_collector
PATH_COMPLETE_FINAL=$PATH_OUTPUT_CDR/number_anasysis/reports
COLLECTION_FINAL_REPORT=$PATH_OUTPUT_CDR/number_anasysis/collection_final_report

#Permission Error
mkdir -p  $PATH_OUTPUT_CDR/number_anasysis/circlewise_report_collector &> /dev/null
rm $PATH_FINAL_COLLECTOR/*  &> /dev/null
mkdir -p $PATH_OUTPUT_CDR/number_anasysis/collection_final_report   &> /dev/null


# export l_date_var=$(date '+%d-%m-%Y_%H%M%S')
# if [ -f /tmp/dummy_$l_date_var.txt ]; then
#     rm -rf /tmp/capturerLog_$l_date_var.txt
# fi

# pidof -x rtCdrCircleAnalysis.sh > /tmp/circleLog_$l_date_var.txt

#        Process_count=`cat /tmp/circleLog_$l_date_var.txt | wc -w`
#  rm -rf /tmp/circleLog_$l_date_var.txt
#echo "$Process_count"

  #echo "Process_count val :[$Process_count]"
#  if [ $Process_count -gt 1 ]; then
#             echo "Error-- Script rtCdrCircleAnalysis.sh is already running on this server. Can't execute mutiple Scripts simultaneously"
...
if [ "$1" = "--help" ] || [ "$1" = "--HELP" ]; then
     ...
     echo -e $GREEN "******************* Welcome to rtCdrCircleAnalysis Script *******************" $NC
     echo -e $BLUE  "The script is used to trigger the rtCdrAnalysis Script in Clusters supplied through cluster_invoke.txt " $NC
        ...
        do
                 ...
                 echo -e $CYAN  " 4. PHONE NUMBER                 : Phone number to search [XXXXXXXXXXXX] " $NC
                 ...
                 echo -e $CYAN  " 5. PHONE NUMBER                 : Phone number to search [XXXXXXXXXXXX] " $NC
#                echo -e $CYAN  " 4. ACTIVE_TRIGGER_CONTROLLER_IP : IP of the server where the circle report to be generated [XX.XX.XX.XX] " $NC
                ...
                echo -e $RED    "3.PHONE_NUMBER "$NC
                 echo -e $RED    "4.MINUTES" $NC
                echo -e $RED   "5.Information About Working of Script " $NC
                 echo -e $RED   "6.EXIT HELP" $NC
...
#  fi
                                             ...
                                             echo -e $GREY   "The script prompts 1 or 2 1=GUI 2=ARGUMENT " $NC
                                                 echo -e $GREY   "Cluster node 1 ip to be written cluster_invoke.txt" $NC
...
#Decide Whether to go with gui or direct invokation
#1st Parameter 1 = gui 2 = Parameter invoke
...
###################################   GUI BUILDING  ######################################################
if (whiptail --title "--- CDR CIRCLE ANALYSIS ---" --yes-button "Option 1" --no-button "Option 2"  --yesno "CHOOSE OPTION  1 = Relative Time  2 = Absolute Time " 10 60) then
...
PHONE_ENTRY=$(whiptail --title "PHONE - NUMBER TO SEARCH " --inputbox "Enter Phone Number" 10 60 919872001093  3>&1 1>&2 2>&3)

 exitstatus=$?
    if [ $exitstatus = 0 ]; then

        #Apply Check on Phone Number

                    if [ "$PHONE_ENTRY" -eq  "$PHONE_ENTRY" ] &> /dev/null
                     then
                        echo "yes" &> /dev/null
                       else
                              echo  -e $RED "--INVALID PHONE_NUMBER ENTERED ----" $NC
                               exit $INVALID_PHONE_NUMBER
                        fi
     ...
     #        [0-9]*) echo "Check pass " &> /dev/null  ;;
      #        *)    echo  -e $RED "--INVALID PHONE_NUMBER ENTERED ----" $NC
       #             exit $INVALID_PHONE_NUMBER
        ...
        echo  -e $RED "Please provide PHONE NUMBER for script to execute --- CHECK --help FOR MORE INFO---" $NC
       #Prevent User from Cancelling Phone Number
       exit $GUI_PHONE_NUMBER_NOT_GIVEN
   fi

## Assign All Parameters in Code
### MINUTES_ENTRY OPTION_ENTRY START_ENTRY END_ENTRY PHONE_ENTRY
      ...
      #Total  = 4
       MINUTES_ENTRY=$3
         ...
         if [ -z "$MINUTES_ENTRY" ]
          ...
          if [ -z "$PHONE_ENTRY" ]; then
                                 echo -e $RED "Please supply PHONE NUMBER to be Searched for :" $NC
                               exit $PNO_NOT_GIVEN
           fi
...
echo -e $GREEN " Passed Parameters :  START_DUARTION [$START_DURATION] END_DURATION [$END_DURATION]  PHONE_NUMBER [$PHONE_NUMBER] ACTIVE_CONTROLLER_IP [$ACTIVE_TRIGGER_CONTROLLER_IP] " $NC



#Enter For how many cluaters need to run :
...
do
   ...
   if [ "X$SELF_IP_ADDRESS" == "X$CLUSTER_ACTIVE_CONTROLLER_IP" ]
         then
        sh $RT_SCRIPT_PATH/rtCdrAnalysis.sh $START_DURATION $END_DURATION $PHONE_NUMBER  $ACTIVE_TRIGGER_CONTROLLER_IP

        else
               /usr/bin/expect<<EOF
                          spawn ssh imsuser@$CLUSTER_ACTIVE_CONTROLLER_IP  $RT_SCRIPT_PATH/rtCdrAnalysis.sh  $START_DURATION $END_DURATION $PHONE_NUMBER $ACTIVE_TRIGGER_CONTROLLER_IP
                                 expect {
...
#After All Trigger Finish May Wait so that All Final Report Are scp then form complete final reports
echo "Preparing complete Final Report :"
# For  all final reports in final_report_collector Append in One File

#Before  Ensure
rm $PATH_COMPLETE_FINAL/* &> /dev/null



CURRTIME=$(date +"%Y-%m-%d@%H:%M:%S:%3N")

touch $PATH_COMPLETE_FINAL/CdrAnalysis_${CURRTIME}_Circle_Report.txt
COMPLETE_PATH_FINAL=$PATH_COMPLETE_FINAL/CdrAnalysis_${CURRTIME}_Circle_Report.txt



#if [ "$(ls -A $PATH_FINAL_COLLECTOR)" ]
#then
 #   echo "Directory Not Empty" &> /dev/null
    #Directory Not Empty
#else
#echo "$PATH_FINAL_COLLECTOR"
  allFiles=`ls  $PATH_FINAL_COLLECTOR/*`
  ZeroSizeFile=`find $PATH_FINAL_COLLECTOR/ -size 0` &> /dev/null

#echo "Path : $PATH_FINAL_COLLECTOR"
#echo "Here : AllFiles : $allFiles ZeroFiles :  $ZeroSizeFile"
   if [ "$allFiles"  == "$ZeroSizeFile" ]
  then
 #  echo "Here"
      #Diretcory is Empty Means No CSV file Found
  echo -e $RED "-------- NOT FOUND IN ANY CLUSTER -----------------" >> $COMPLETE_PATH_FINAL $NC
...
do
   ...
   filesize=`stat -c %s  ${PATH_FINAL_COLLECTOR}/$file `

   echo -e $RED  "*********************************************************" >> $COMPLETE_PATH_FINAL $NC
#     echo "                                                         " >> $COMPLETE_PATH_FINAL
#      echo "                                                         " >> $COMPLETE_PATH_FINAL
      #echo "                                                         " >> $COMPLETE_PATH_FINAL
   if [ $filesize == 0 ]
   then
      CLUSTER_IP=`echo $file | awk -F "_" '{print $3}'`
      echo -e $RED "CLUSTER IP : [$CLUSTER_IP]  ----   NOT FOUND ------ "  >> $COMPLETE_PATH_FINAL $NC
   else
